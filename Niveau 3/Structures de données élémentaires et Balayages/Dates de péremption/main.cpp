...
int main()
{
  ...
  for (int i{0}; i < nbOperation; i++)
  {
    ...
    {
      stock [j].deltaStj += quantite; //std::cout << "stock [" << j << "].deltaStj = " << stock [j].deltaStj << "\n";
      if (stock [j].deltaStj < 0)
      {
        int depassement = stock [j].deltaStj;
        stock.erase(stock.begin() + j);
        j--;
        while (depassement < 0)
        {
          stock [j].deltaStj += depassement;
          depassement = stock [j].deltaStj;
          if (depassement < 0)
          {
            stock.erase(stock.begin() + j);
            j--;
          }
          else if (stock [j].deltaStj == 0)
          {
           stock.erase(stock.begin() + j);
           j--;
          }
